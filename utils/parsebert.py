...
# [1a] Sets up tensors for BERT processing
def h_tensors(max_rows, max_cols, pad_length, tokenizer):
    """
    Initializes tensors for tokenized data, attention masks, and metadata
    for BERT-based processing.
    ...
    Args:
        ...
        tokenizer: HuggingFace tokenizer for BERT-based processing.
    ...
    minimal_ids = [tokenizer.cls_token_id, tokenizer.sep_token_id] + [
...
def h_cleanmetadata(row, col, x_tok, y_tok, x_masks):
    """
    ...
    Args:
        ...
        x_masks (torch.Tensor): Tensor storing attention masks for each cell.

    Returns:
        tuple: Updated (x_tok, y_tok, x_masks) tensors after cleaning.
    """
...
# [1c] Encodes cell value using BERT tokenizer
def h_encode(cell_value, row, col, x_tok, x_masks, pad_length, tokenizer):
    """
    Encodes cell value using BERT tokenizer and updates tensors.
    ...
    Args:
        ...
        x_masks (torch.Tensor): Mask tensor to update
...
def xls_mergedata(sheet, x_tok, y_tok, x_masks, max_rows, max_cols):
    """
...
def process_xls(file_path, max_rows=100, max_cols=100, pad_length=32, tokenizer=None):
    ...
    for row, col in itertools.product(
        ...
        x_tok, y_tok, x_masks = h_cleanmetadata(row, col, x_tok, y_tok, x_masks)

    # Propagate merged cell metadata
    x_tok, y_tok, x_masks = xls_mergedata(
        sheet, x_tok, y_tok, x_masks, max_rows, max_cols
    )
...
def xlsx_mergedata(sheet, x_tok, y_tok, x_masks, max_rows, max_cols):
    """
    Handles processing and propagating data across merged cells.
...
def process_xlsx(file_path, max_rows=100, max_cols=100, pad_length=32, tokenizer=None):
    ...
    Args:
        ...
        tokenizer: BERT tokenizer object for processing.

    Returns:
        tuple: A tuple (x_tok, x_masks, y_tok) where:
            - x_tok (torch.Tensor): A 3D tensor of tokenized cell values.
            - x_masks (torch.Tensor): A 3D tensor of attention masks.
...
def compare_tensors(tensor1, tensor2):
    ...
    if tensor1.device != tensor2.device:
        print(
            f"Device Mismatch: Moving tensor1 from {tensor1.device} to {tensor2.device}"
        )
