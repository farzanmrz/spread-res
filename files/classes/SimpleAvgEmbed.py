...
class SimpleAvgEmbed(nn.Module):

	def __init__( self, embedding_matrix, dropout_rate = 0.05 ):
		super(SimpleAvgEmbed, self).__init__()
...
# # Flatten the 2D grid of cells into a single dimension (rows x cols)
# flat_x = x.view(x.shape[0], -1, x.shape[3])  # [batch_size, num_cells, sequence_length]
#
# # Embed and average all cells at once
# flat_embeddings = self._embed(flat_x).mean(dim=2)  # [batch_size, num_cells, embedding_dim]
#
# # Dropout and prediction for all cells at once
# flat_predictions = self._pred(self._drop(flat_embeddings)).view(x.shape[0], x.shape[1], x.shape[2])  # [batch_size, rows, cols]
#
